...
/* Skills buttons */
const skillButtons = document.querySelectorAll('#aboutme-container ul button');
...
function handleClick() {
    ...
    oldInfo = document.querySelector(`#aboutme-container-skills-info div[name='${this.textContent}'`);
...
const tooltipContent = {
    ...
    "C#/Unity": {
        "proficiency": "Beginner",
        "description": "I have used C# little bit in school when making unity games and terraria plugin. But otherwise i dont really have more experience but i believe i would learn it fast since i do have knowledge of java."
    },
    "MySQL": {
        "proficiency": "Beginner",
...
function toggleTooltip(event) {
    ...
    const contentName = event.target.textContent.replace(/\s*🛈$/, '');
    const contentData = tooltipContent[contentName];

    if (contentData) {
        const description = contentData.description ? `: ${contentData.description}` : '';
        const tooltipText = `${contentData.proficiency}${description}`;
        
        skillText.textContent = skillText.textContent === tooltipText ? "" : tooltipText;
    }
}